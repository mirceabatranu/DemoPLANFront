    ...
    def analyze_pdf(self, file_content: bytes, filename: str = None) -> PDFAnalysisResult:
        ...
        try:
            ...
            result = PDFAnalysisResult(
                ...
                tables_extracted=ocr_metadata.get('tables_extracted', []),
...
class GeometryFirstExtractor:
    ...
    def _extract_spatial_boundaries(self, doc: Drawing) -> List[SpatialBoundary]:
        ...
        boundaries = []
        try:
            for i, entity in enumerate(doc.modelspace().query('LWPOLYLINE[is_closed==True]')):
                ...
                if len(vertices) >= 3:
                    ...
                    if self.min_room_area <= area <= self.max_room_area:
                        boundaries.append(SpatialBoundary(
                            boundary_id=f"boundary_{i}", vertices=vertices, area=area,
                            centroid=self._calculate_centroid(vertices),
                            confidence=self._calculate_boundary_confidence(area, vertices),
                            associated_texts=[]
                        ))
            
            for i, entity in enumerate(doc.modelspace().query('POLYLINE[is_closed==True]')):
                try:
                    ...
                    if len(vertices) >= 3:
                        ...
                        if self.min_room_area <= area <= self.max_room_area:
                            boundaries.append(SpatialBoundary(
                                boundary_id=f"polyline_{i}", vertices=vertices, area=area,
                                centroid=self._calculate_centroid(vertices),
                                confidence=self._calculate_boundary_confidence(area, vertices),
                                associated_texts=[]
                            ))
            ...
            logger.warning(f"Error extracting boundaries: {e}")
...
class DXFAnalyzer:
    ...
    def analyze(self, file_content: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Failed to analyze DXF file: {e}", "status": "error"}
    ...
    def _create_room_breakdown(self, boundaries: List[SpatialBoundary], texts: List[FilteredText]) -> List[Dict[str, Any]]:
        ...
        for boundary in boundaries:
            ...
            room_breakdown.append({
                "room_id": boundary.boundary_id, "room_type": room_type,
                "romanian_name": self._get_romanian_room_name(room_type),
                "area": round(boundary.area, 2), "location": boundary.centroid,
                "confidence": round(confidence, 2), "is_validated": bool(boundary.associated_texts),
