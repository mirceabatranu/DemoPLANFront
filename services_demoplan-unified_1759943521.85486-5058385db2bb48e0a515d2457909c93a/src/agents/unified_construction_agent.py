...
class UnifiedConstructionAgent:
    ...
    async def analyze_project(
        ...
        try:
            ...
            for file_info in files:
                ...
                # Save this single file analysis to the subcollection
                await self.firestore_service.save_file_analysis(
    ...
    async def _process_single_file(self, file_info: Dict[str, Any], context: AgentContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if document_result.document_type == "dxf" and document_result.dxf_analysis:
                ...
                single_file_result["analysis_summary"] = {
                    "type": "txt",
                    "requirements": len(analysis_dict.get('requirements', []))
                }
    ...
    async def _consolidate_analysis(
        ...
        has_materials = (
            context.project_data.get("pdf_materials") or
            context.project_data.get("txt_keywords") or
            file_analysis.get("dxf_analysis", {}).get("dxf_analysis", {}).get("specification_analysis", {}).get("materials_count", 0) > 0
        )
...
- Detaliază ferestre și uși (dimensiuni, tipuri, materiale)
        ...
        for idx, file_ref in enumerate(file_references, 1):
            ...
            if analysis_summary.get("type") == "dxf":
                ...
                if door_window_schedule:
                    ...
                    if windows:
                        file_context_parts.append(f"   - **Ferestre (TOATE {len(windows)} bucăți):**")
